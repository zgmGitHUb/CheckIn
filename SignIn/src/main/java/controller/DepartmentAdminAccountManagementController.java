...
import service.adminAccountManagementService.DepartmentAdminAccountManagementService;
...
public class DepartmentAdminAccountManagementController {
    @Autowired
    private DepartmentAdminAccountManagementService departmentAdminAccountManagementService;
    ...
    public @ResponseBody List<TbEmployee> queryEmployee(TbEmployee tbEmployee)
    {
        ...
        if(employeeId==null||employeeId=="")
            return null;
        List<TbEmployee> employeeList= departmentAdminAccountManagementService.queryEmployee(tbEmployee);
...
//            System.out.println(session.getAttribute("Verification"));
        ...
        if(session.getAttribute("Verification").equals(code2)&& session.getAttribute("EmployeeId").equals(employeeId)) {
            if (departmentAdminAccountManagementService.changeTelephone(tbEmployee))json.put("status",true);
            else json.put("status",false);
        }
        else json.put("status",false);
        }
    ...
    {   System.out.println("test sendEmailCode Email:"+tbEmployee.getEmail());
        JSONObject json = new JSONObject();
        try {
            String UUID = bindEmail.sendEmailCode(tbEmployee.getEmployeeid(),tbEmployee.getEmail(),"AdminAccountManagement");
...
//            System.out.println(sUUID);
        ...
        if(sEmployeeId.equals(employeeId) && sEmail.equals(email) && sUUID.equals(UUID)) {
            if(bindEmail.checkEmailCode(createTime)) {
                if(departmentAdminAccountManagementService.changeEmail(tbEmployee))
    ...
    {   System.out.println("test changePassword");
        ...
        if(password==null||password=="") {
            ...
            if (departmentAdminAccountManagementService.changePassword(tbEmployee))json.put("status",true);
            else json.put("status",false);
        }
        return json;
    }
    ...
    {   System.out.println("test verification");
        ...
        if(password==null||password=="") {
            ...
            List<TbEmployee> employeeList = departmentAdminAccountManagementService.verification(tbEmployee);
